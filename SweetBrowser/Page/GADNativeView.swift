...
//  SwiftBrowser
//
//  Created by yangjian on 2023/9/22.
//
...
struct GADNativeView: UIViewRepresentable {
    ...
    func makeUIView(context: UIViewRepresentableContext<GADNativeView>) -> UIView {
        return model.view
    }
    
    func updateUIView(_ uiView: UIView, context: UIViewRepresentableContext<GADNativeView>) {
...
extension UINativeAdView {
    func setupUI() {
        ...
        let width = self.bounds.size.width - iconImageView.frame.maxX - 12 - 10 - 25 - 16 - 72 - 10
        ...
        installLabel.frame = CGRectMake(adView.frame.maxX + 10, 18, 72, 40)
    ...
    func refreshUI(ad: GADNativeAd? = nil) {
        self.nativeAd = ad
        self.backgroundColor =  UIColor(named: "#FFEDCF")
        self.adView.image = UIImage(named: "ad_tag")
        self.installLabel.setTitleColor(.black, for: .normal)
        self.installLabel.backgroundColor = UIColor(named: "#7EFF71")
